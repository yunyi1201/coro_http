...
struct timer_node {
  ...
  struct rb_root root; /* coroutines with same timeout */
  long long timeout;
};

struct coroutine {
  struct list_head list; /* idle or active linked list */
  struct rb_node node;   /* timer node */
  int coro_id;           /* coroutine idertifier */
  struct context ctx;
...
static struct coro_scheduler sched;

static struct coroutine *find_in_timer(struct timer_node *tm_node,
                                       int coro_id) {
...
static void move_to_inactive_tree(struct coroutine *coro) {
  ...
  while (*newer) {
    struct timer_node *tm_node = container_of(*newer, struct timer_node, node);
...
void *current_coro() { return sched.current; }
