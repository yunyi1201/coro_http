...
static inline enum http_method get_request_method(str_t *method_name) {
  ...
  switch (method_name->len) {
    case 3:
      if (CMP3(p, 'G', 'E', 'T')) return HTTP_GET;
      if (CMP3(p, 'P', 'U', 'T')) return HTTP_PUT;
      return HTTP_UNKNOWN;
    case 4:
      if (p[1] == 'O') {
        if (CMP30(p, 'P', 'O', 'S', 'T')) return HTTP_POST;
        if (CMP30(p, 'C', 'O', 'P', 'Y')) return HTTP_COPY;
        if (CMP30(p, 'M', 'O', 'V', 'E')) return HTTP_MOVE;
        if (CMP30(p, 'L', 'O', 'C', 'K')) return HTTP_LOCK;
        return HTTP_UNKNOWN;
      }
      if (CMP4(p, 'H', 'E', 'A', 'D')) return HTTP_HEAD;
      return HTTP_UNKNOWN;
    case 5:
      if (CMP5(p, 'M', 'K', 'C', 'O', 'L')) return HTTP_MKCOL;
      if (CMP5(p, 'P', 'A', 'T', 'C', 'H')) return HTTP_PATCH;
      if (CMP5(p, 'T', 'R', 'A', 'C', 'E')) return HTTP_TRACE;
      return HTTP_UNKNOWN;
    case 6:
      if (CMP6(p, 'D', 'E', 'L', 'E', 'T', 'E')) return HTTP_DELETE;
      if (CMP6(p, 'U', 'N', 'L', 'O', 'C', 'K')) return HTTP_UNLOCK;
      return HTTP_UNKNOWN;
    case 7:
      if (CMP7(p, 'O', 'P', 'T', 'I', 'O', 'N', 'S', ' ')) return HTTP_OPTIONS;
      return HTTP_UNKNOWN;
    case 8:
      if (CMP8(p, 'P', 'R', 'O', 'P', 'F', 'I', 'N', 'D')) return HTTP_PROPFIND;
      return HTTP_UNKNOWN;
    case 9:
...
int http_parse_request_line(struct http_request *r, struct buffer *b) {
    ...
    switch (state) {
      ...
      case S_method:
        if (ch == ' ') {
          ...
          if (HTTP_UNKNOWN & r->method) return -2;
          state = S_spaces_before_uri;
          break;
        }

        if ((ch < 'A' || ch > 'Z') && ch != '_') return -3;
        break;
      ...
      case S_major_digit:
        ...
        if (ch < '0' || ch > '9') return -20;
        r->http_major = r->http_major * 10 + ch - '0';
        break;
      /* first digit of minor HTTP version */
      case S_first_minor_digit:
        if (ch < '0' || ch > '9') return -21;
        r->http_minor = ch - '0';
        state = S_minor_digit;
        break;
  ...
  if (r->exten.p) r->exten.len = r->uri.p + r->uri.len - r->exten.p;
  b->pos = p + 1;
  if (r->http_minor == 9) r->http_major = 0;
  r->http_version = r->http_major * 1000 + r->http_minor;
  r->state = S_start;
  if (r->http_version == 9 && r->method != HTTP_GET) return -25;
