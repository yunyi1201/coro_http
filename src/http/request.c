...
static int http_process_host(struct http_request *r) {
  ...
  for (size_t i = 0; i < r->header_value.len; i++) {
    ...
    switch (ch) {
      ...
      default:
        if (ch == '/') return -2;
        if (ch >= 'A' && ch <= 'Z') r->header_value.p[i] = tolower(ch);
        break;
    }
  }

  if (dot_pos == host_len - 1) host_len--;
...
static int http_process_request_header(struct http_request *r) {
  ...
  for (;;) {
    ...
    switch (ret) {
      case 0: /* successful */
        if (handle_http_request_header(r)) return -1;
        continue;
      case 1: /* continue */
...
static void __http_request_handler(struct http_request *r) {
  ...
  if (request_line_handler.method && request_line_handler.method(r)) return;

  if (request_line_handler.uri && request_line_handler.uri(r)) return;

  if (request_line_handler.http && request_line_handler.http(r)) return;
...
static unsigned hash_key_lc(unsigned char *data, size_t len) {
  ...
  for (size_t i = 0; i < len; i++) key = HASH(key, tolower(data[i]));
...
void http_request_init(size_t client_header_buffer_kbytes,
  ...
  http_request_cache = memcache_create(client_header_size, worker_connections);
  ...
  if (body_handler) request_body_handler = body_handler;
}
